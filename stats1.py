...
def calculate_marks(data):
    print(f"mean for the given data is :- [{np.mean(marks)}]")
    print(f"median for the given data is :- [{np.median(marks)}]")
    print(f"mode for the given data is :-[{statistics.mode(marks)}]")
    print(f"standard deviation for the given data is :- [{np.std(marks)}]")
...
from scipy.stats import poisson
#Rate of entering word =77 per minute
#error rate= 6/60=0.1 per minute
#Nmber of error per word=0.1/77
unit_mu=0.1/77
def mu(n):
    return n * unit_mu
...
def fun(z, b=-np.inf):
    z = (z-mean)/std
    return integrate.quad(norm.pdf,b,z)[0]
print(f"Probability that current > 13mA is: {1-fun(13)}")
print(f"Probability that current is between 9 mA and 11 mA is : {1-fun(11,b=9)}")
...
mean_dia=0.2508
std_dia=0.0005
def calculate(mean,std,a,b) :
  #gives P(Z<=x)
  a=(a-mean)/std
  b=(b-mean)/std
print(f"Proportion of shafts with dia in range of 0.2485<d<0.2515 when mean diameter:{0.2508,calculate(0.2508,0.0005,0.2485,0.2515)}")
print(f"Proportion of shafts with dia in range of 0.2485<d<0.2515 when mean diameter:{0.2500,calculate(0.2500,0.0005,0.2485,0.2515)}")
